...
pub use pdot_runtime;
pub use runtime_rococo;
pub use runtime_westend;

native_executor_instance!(
	pub PolkadotExecutor,
	pdot_runtime::api::dispatch,
	pdot_runtime::native_version,
...
>
{
	...
	if config.chain_spec.is_rococo() {
		...
		let service::PartialComponents { client, backend, import_queue, task_manager, .. }
			= new_partial::<pdot_runtime::RuntimeApi, PolkadotExecutor>(config, jaeger_agent)?;
...
pub fn build_light(config: Configuration) -> Result<(TaskManager, RpcHandlers), Error> {
	if config.chain_spec.is_rococo() {
		...
		new_light::<pdot_runtime::RuntimeApi, PolkadotExecutor>(config)
	}
}
...
pub fn build_full(
	...
	if config.chain_spec.is_rococo() {
		...
		new_full::<pdot_runtime::RuntimeApi, PolkadotExecutor>(
			config,
			is_collator,
			grandpa_pause,
			jaeger_agent,
